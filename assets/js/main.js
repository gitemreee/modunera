...
(() => {
  ...
  qsa('form[data-lead-form]').forEach(form=>form.addEventListener('submit',async e=>{e.preventDefault();const fd=Object.fromEntries(new FormData(form));const lead={...fd,source:location.pathname,createdAt:new Date().toISOString()};
  // The lead now reaches a server: Netlify Forms accepts an URL-encoded POST to
  // any path when form-name matches a build-time-registered form. Before this,
  // preventDefault() meant NOTHING was ever submitted anywhere — a buyer who
  // closed the WhatsApp tab left no trace. WhatsApp stays as the second channel.
  if(form.getAttribute('data-netlify')){delete fd['bot-field'];try{await fetch('/',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:new URLSearchParams({'form-name':form.getAttribute('name')||'projektanfrage',...fd}).toString()})}catch(err){console.warn('form submission failed',err)}}const leads=JSON.parse(localStorage.getItem('mcLeads')||'[]');leads.push(lead);localStorage.setItem('mcLeads',JSON.stringify(leads));const c=await integrationPromise;if(c.crmEndpoint){try{await fetch(c.crmEndpoint,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(lead)})}catch(err){console.warn('CRM submission failed',err)}}track('lead_form_submit',{form_id:form.id||form.getAttribute('data-lead-form')||'unnamed',fields:Object.keys(fd).length});toast(LEAD_NOTE[document.documentElement.lang?.slice(0,2)]||LEAD_NOTE.de);const msg='MODUNERA Anfrage\n'+Object.entries(fd).map(([k,v])=>`${k}: ${v}`).join('\n');setTimeout(()=>window.open('https://wa.me/905535435342?text='+encodeURIComponent(msg),'_blank'),450)}));
