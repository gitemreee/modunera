...
BASE_URL = 'https://www.mctiny.com/de/'
TODAY = '2026-07-20'
...
def location_faqs(p: Place) -> list[tuple[str,str]]:
    return [
        ...
        (f'Kann ein MC Tiny House nach {p.name} geliefert werden?', f'Eine Lieferung nach {p.name} wird projektbezogen geprüft. Entscheidend sind Abmessungen, Gewicht, Route, Zufahrt, Rangierfläche und Entladepunkt. Die finale Transportplanung erfolgt nach Standortfreigabe.'),
        ...
        (f'Ist ein Tiny House in {p.name} winterfest?', f'Die MC-Tiny-Modelle können für eine ganzjährige Nutzung konfiguriert werden. Dämmung, Fenster, Wärmebrücken, Lüftung und Heizleistung müssen jedoch auf das konkrete Klima und Grundstück in {p.name} abgestimmt werden.'),
...
def location_page(p: Place, places: list[Place]) -> str:
    ...
    title=f'Tiny House in {p.name} kaufen | Hersteller & Lieferung | MC Tiny'
    ...
    schema=[
        {'@context':'https://schema.org','@type':'WebPage','name':title,'url':canonical,'description':desc,'about':{'@type':'Product','name':f'Tiny House für {p.name}','brand':{'@type':'Brand','name':'MC Tiny'}}},
...
def state_page(state: str, group: list[Place]) -> str:
    ...
    title=f'Tiny House {state}: Hersteller, Modelle & Lieferung | MC Tiny'
...
def locations_index(places: list[Place], states: dict[str,list[Place]]) -> str:
    ...
    title='Tiny House Standorte Deutschland | 7.000+ regionale Seiten | MC Tiny'
...
def blog_sections(topic: tuple[str,str,str,str], angle: str) -> list[tuple[str,str]]:
    ...
    if angle=='leitfaden':
        return [
            ...
            ('Technische Spezifikation belastbar formulieren', 'Ein Angebot sollte nicht nur schöne Bilder enthalten, sondern klare Angaben zu Abmessungen, Gewicht, Tragstruktur, Fahrgestell, Dämmung, Fenstern, Elektroinstallation, Lüftung, Heizung, Oberflächen und Einbauten. Bei MC Tiny bilden feuerverzinkte Stahlkonstruktion, Stahlfahrgestell, wärmeisolierte PVC-Elemente, doppelt gehärtetes Thermopane und halogenfreie 220-Volt-Installation eine technische Basis, die projektbezogen ergänzt wird.'),
...
def blog_page(topic, angle, index) -> tuple[str,str,str,str]:
    ...
    schema=[
        {'@context':'https://schema.org','@type':'BlogPosting','headline':title,'description':desc,'datePublished':pub,'dateModified':TODAY,'author':{'@type':'Organization','name':'MC Tiny Redaktion'},'publisher':{'@id':BASE_URL+'#organization'},'mainEntityOfPage':canonical,'image':BASE_URL+'assets/images/gallery/'+image,'articleSection':FAQ_CATEGORIES.get(cat,cat)},
    ...
    for cat,items in subjects.items():
        for i,sub in enumerate(items):
            if cat=='modelle':
                ...
                a=f'{sub} ist Teil der MC-Tiny-Modell- oder Ausstattungslogik. Entscheidend sind Personenanzahl, Nutzung, gewünschte Raumtrennung, Standort, technische Ausstattung und Gewicht. Der Modellvergleich und die technische Beratung grenzen die passende Konfiguration ein.'
            elif cat=='recht':
                ...
                a=f'{sub} sollte mit Zielgruppe, Standort, Genehmigung, Investitionskosten, Auslastung, Reinigung, Wartung und konservativer Erlösplanung bewertet werden. MC Tiny kann Produkt- und Serienkonfiguration unterstützen; Wirtschaftlichkeitsannahmen bleiben Betreiberverantwortung.'
            else:
                q=f'Wie funktioniert {sub} bei MC Tiny?'
...
def faq_page(faqs: list[dict]) -> str:
    root='../'; canonical=BASE_URL+'faq/'; title='Tiny House FAQ: 160 Antworten zu Kauf, Technik & Lieferung | MC Tiny'; desc='160 klare Antworten zu Tiny-House-Kauf, Preisen, Genehmigung, Modellen, Winterfestigkeit, Transport, Nachhaltigkeit, Airbnb und Service.'
...
def blog_index(entries: list[dict]) -> str:
    root='../'; canonical=BASE_URL+'blog/'; title='Tiny House Ratgeber: Planung, Preise, Technik & Business | MC Tiny'; desc=f'{len(entries)} fundierte Tiny-House-Ratgeber zu Kauf, Standort, Technik, Energie, Gestaltung, Transport, Nachhaltigkeit und Betreiberprojekten.'
    schema=[{'@context':'https://schema.org','@type':'CollectionPage','name':title,'url':canonical,'description':desc}]
...
def main():
    ...
    knowledge={
        '@context':'https://schema.org','@graph':[
            {'@type':'Organization','@id':BASE_URL+'#organization','name':'MC Tiny','url':BASE_URL,'telephone':PHONE_DISPLAY,'description':'Tiny-House-Hersteller mit eigener Produktion für Deutschland und Europa.','areaServed':['DE','AT','CH','NL','BE','FR','SE','NO','DK','FI']},
            *[{'@type':'Product','@id':BASE_URL+f'modelle/{m["id"]}/#product','name':m['name'],'brand':{'@type':'Brand','name':'MC Tiny'},'description':m['positioning'],'url':BASE_URL+f'modelle/{m["id"]}/'} for m in MODELS]
        ]
    }
    ...
    write(SITE/'feed.xml',f'<?xml version="1.0" encoding="UTF-8"?><feed xmlns="http://www.w3.org/2005/Atom"><title>MC Tiny Ratgeber</title><id>{BASE_URL}blog/</id><updated>{TODAY}T00:00:00Z</updated>{feed_items}</feed>')
