    ...
    items: [
      { label: "Tiny Houses", cols: [
        ...
        ["Auswählen und planen", [
          ...
          ["konfigurator/", "Design Studio", "Live konfigurieren"],
    ...
    items: [
      { label: "Tiny houses", cols: [
        ...
        ["Choose and plan", [
          ...
          ["konfigurator/", "Design studio", "Configure live"],
...
/* The three locale blogs are generated by tools/build-modunera-depth.mjs from
   data/blog-<code>.json. The menu reads the same files so the two layers cannot
   drift: a renamed category shows up in the navigation on the next build. */
const LOCALE_BLOG_DATA = Object.fromEntries(await Promise.all(["nl", "da", "fr"].map(async (code) =>
  [code, JSON.parse(await readFile(join(ROOT, `data/blog-${code}.json`), "utf8"))])));
const LOCALE_BLOG_SLUGS = Object.fromEntries(Object.entries(LOCALE_BLOG_DATA).map(([code, b]) =>
  [code, b.categories.map((c) => [c.slug, c.name])]));
const LOCALE_BLOG_LABELS = {
  nl: { all: "Alle onderwerpen", sub: "Negen onderwerpen" },
  da: { all: "Alle emner", sub: "Ni emner" },
  fr: { all: "Tous les sujets", sub: "Neuf sujets" },
};
    ...
    items: [
      { label: cfg.nav.models, cols: [
        ...
        [cfg.labels.plan ?? cfg.nav.models, [
          ...
          ["konfigurator/", "Design Studio", "MODUNERA"],
      ...
      // each of the three locales has its own blog, written for its own market
      // rather than translated — data/blog-nl.json, blog-da.json, blog-fr.json
      { label: "Blog", menu: [
        [`${code}/blog/`, LOCALE_BLOG_LABELS[code].all, LOCALE_BLOG_LABELS[code].sub],
        ...LOCALE_BLOG_SLUGS[code].slice(0, 5).map(([slug, name]) => [`${code}/blog/${slug}/`, name, "Tiny House"]),
      ]},
...
function modelComparisonPage(pricing, lang) {
  ...
  const faqs = de
    ? [
        ...
        ["Kann ich einen Grundriss anpassen?", "Grundriss-, Material- und Möbelanpassungen sind je nach Projekt möglich, weil Rahmen, Hülle, Ausbau und Möbel aus eigener Produktion kommen. Umfang und Auswirkung auf Preis und Termin werden vor der Bestellung schriftlich festgehalten."],
...
function priceComparisonPage(pricing, lang) {
  ...
  const faqs = de
    ? [
        ...
        ["Sind Zoll und Einfuhrumsatzsteuer enthalten?", "Nein. Zoll, Einfuhrumsatzsteuer und lokale Abgaben hängen von Warenwert, Einfuhrweg und Status im Zielland ab und werden für jede Sendung einzeln ermittelt. Die EU-Türkei-Zollunion deckt nicht jede Warengruppe gleich ab."],
  ...
  return (
    ...
    `<div class="answer-box"><strong>${de ? "Wie diese Zahlen entstehen" : "How these figures are produced"}</strong><p>${de ? "Die Lieferindikationen entsprechen exakt den Tarifen, die der MODUNERA Konfigurator rechnet. Für Luxemburg ist derzeit kein Tarif hinterlegt; die Route wird für jedes Projekt einzeln kalkuliert. Das Einstiegsbudget kombiniert das günstigste Basismodell mit der Lieferindikation und enthält keine Abgaben, kein Fundament und keine Anschlüsse." : "The delivery indications match the tariffs the MODUNERA configurator applies. No tariff is stored for Luxembourg at present; that route is calculated per project. The entry budget combines the most affordable base model with the delivery indication and excludes duties, foundation and connections."}</p></div>` +
...
const MARKET_GUIDES = [
  ...
  },
  {
    ...
    de: {
      ...
      faqs: [
        ["Ist ein MODUNERA Tiny House winterfest?", "Die Vier-Jahreszeiten-Ausführung ist darauf ausgelegt. Entscheidend ist, dass Dämmung, Fenster, Lüftung und Heizung auf das Klima am konkreten Standort abgestimmt werden – das wird je Projekt festgelegt."],
...
async function rewriteWhatsapp() {
  ...
  for (const file of files) {
    ...
    /* The dock's markup depends on assets/css/styles.css. On a page that does not
       load that stylesheet — a self-contained page without it — the
