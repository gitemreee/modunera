...
const COUNTRIES = {
  ...
  DK: {
    ...
    legalDe: "Für neue Gebäude, Erweiterungen und Nutzungsänderungen ist in Dänemark grundsätzlich eine Genehmigung erforderlich. Zuständig ist die jeweilige Kommune; kleinere Vorhaben können Ausnahmen haben, müssen aber dennoch die geltenden Regeln einhalten.",
...
const SERVICE_PAGES = [
  { slug: "modulbau", enSlug: "modular-buildings", de: "Modulbau & Modulhäuser", en: "Modular buildings", image: "mc3-exterior.webp", deIntro: "Erweiterbare Raummodule für Wohnen, Hospitality, Büro und Gewerbe – projektbezogen geplant und transportfähig gefertigt.", enIntro: "Expandable modules for residential, hospitality, office and commercial use, engineered for the project and manufactured for transport." },
...
const slugify = (value) => String(value).replaceAll("ß", "ss").replaceAll("ä", "ae").replaceAll("ö", "oe").replaceAll("ü", "ue").replaceAll("Ä", "Ae").replaceAll("Ö", "Oe").replaceAll("Ü", "Ue").normalize("NFKD").replace(/[\u0300-\u036f]/g, "").toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "") || "ort";
...
function countryFaqs(country, lang) {
  if (lang === "de") return [
    [`Liefert MODUNERA Tiny Houses nach ${country.de}?`, `Ja, Lieferungen nach ${country.de} werden projektbezogen angeboten. Modell, Abmessungen, Gewicht, Route, Zollstatus, letzte Zufahrt und Entladung müssen vorab freigegeben werden.`],
...
function servicePage(service, lang) {
  ...
  const faq = isDe ? [["Bietet MODUNERA diese Leistung zusätzlich zu Tiny Houses an?", "Ja. Tiny Houses bleiben das Hauptgeschäft; dieses Leistungsfeld ergänzt private, gewerbliche und Hospitality-Projekte."], ["Ist Sonderanfertigung möglich?", "Ja, nach technischer, statischer, materialbezogener und logistischer Prüfung."], ["Kann nach Europa geliefert werden?", "Lieferung und Montage werden je nach Abmessung, Route, Zoll, Baustelle und Leistungsumfang projektbezogen angeboten."]] : [["Is this service offered alongside tiny houses?", "Yes. Tiny houses remain the core business; this capability supports private, commercial and hospitality projects."], ["Is bespoke production possible?", "Yes, subject to technical, structural, material and logistics review."], ["Can MODUNERA deliver across Europe?", "Delivery and installation are offered per project depending on dimensions, route, customs, site and scope."]];
  ...
  return head({ file, lang, title, description, image: service.image, alternateDe: canonicalFor(isDe ? file : counterpart), alternateEn: canonicalFor(isDe ? counterpart : file), schema: [{ "@context": "https://schema.org", "@type": "Service", name, provider: { "@type": "Organization", name: "MODUNERA", url: BASE }, areaServed: ["DE", "NL", "DK", "LU", "CH"], description }, faqSchema(faq)] }) + nav(root, lang) + `<main id="main"><header class="location-hero"><img src="${root}assets/images/gallery/${service.image}" alt="${esc(name)}"><div class="location-hero-overlay"></div><div class="container location-hero-content"><div class="eyebrow">MODUNERA · ${isDe ? "Weitere Lösungen" : "Additional solutions"}</div><h1>${esc(name)}</h1><p>${esc(intro)}</p><div class="hero-actions"><a class="btn btn-primary" href="${waLink(message)}" target="_blank" rel="noopener">${isDe ? "Projekt per WhatsApp starten" : "Start on WhatsApp"}</a><a class="btn btn-light" href="${root}${isDe ? "index.html#modelle" : "en/#models"}">${isDe ? "Tiny Houses ansehen" : "View tiny houses"}</a></div></div></header><section class="section section-tight"><div class="container"><div class="answer-box"><strong>${isDe ? "Einordnung" : "Positioning"}</strong><p>${isDe ? `MODUNERA ist zuerst Tiny-House-Hersteller. ${service.de} erweitert die Fertigung für Projekte, die mehr Fläche, besondere Tragstrukturen, ebenerdige Grundrisse oder eigenständige Möbelpakete benötigen.` : `MODUNERA is a tiny-house manufacturer first. ${service.en} extends production for projects that need more space, special structural systems, single-level layouts or standalone furniture packages.`}</p></div></div></section><section class="section"><div class="container"><div class="process-grid"><div><span>01</span><h3>${isDe ? "Briefing" : "Brief"}</h3><p>${isDe ? "Ort, Nutzung, Maße, Menge, Budget und Termin." : "Location, use, dimensions, quantity, budget and timing."}</p></div><div><span>02</span><h3>${isDe ? "Machbarkeit" : "Feasibility"}</h3><p>${isDe ? "Statik, Material, Technik, Normen und Transport." : "Structure, materials, services, standards and transport."}</p></div><div><span>03</span><h3>${isDe ? "Bemusterung" : "Specification"}</h3><p>${isDe ? "Grundriss, Oberflächen, Details und Freigaben." : "Layout, finishes, details and approvals."}</p></div><div><span>04</span><h3>${isDe ? "Fertigung" : "Production"}</h3><p>${isDe ? "Qualitätskontrolle, Dokumentation und Lieferung." : "Quality control, documentation and delivery."}</p></div></div></div></section><section class="section section-soft"><div class="container"><div class="faq-list">${faqMarkup(faq)}</div></div></section></main>` + footer(root, lang);
}
...
function servicesIndex(lang) {
  ...
  const root = rootFor(file);
  const cards = SERVICE_PAGES.map((service) => `<a class="journey-card" href="${isDe ? service.slug : service.enSlug}/"><div><span class="num">${String(SERVICE_PAGES.indexOf(service) + 1).padStart(2, "0")}</span><h3>${esc(service[isDe ? "de" : "en"])}</h3><p>${esc(service[isDe ? "deIntro" : "enIntro"])}</p></div><span class="arrow">↗</span></a>`).join("");
  const title = isDe ? "Modulbau, Stahlbau, Bungalows & Möbel nach Maß | MODUNERA" : "Modular, steel, bungalows & bespoke furniture | MODUNERA";
  const description = isDe ? "Weitere MODUNERA Fertigungskompetenzen neben Tiny Houses: Modulbau, Stahlkonstruktionen, Bungalows und maßgefertigte Möbel." : "MODUNERA capabilities alongside tiny houses: modular buildings, steel structures, bungalows and bespoke furniture.";
  return head({ file, lang, title, description, alternateDe: BASE + "leistungen/", alternateEn: BASE + "en/services/" }) + nav(root, lang) + `<main id="main"><header class="page-hero"><div class="container"><div class="eyebrow">${isDe ? "Tiny House bleibt Kern" : "Tiny house remains core"}</div><h1>${isDe ? "Mehr Fertigung für größere Ideen." : "More manufacturing for bigger ideas."}</h1><p>${esc(description)}</p></div></header><section class="section"><div class="container"><div class="journey-grid">${cards}</div></div></section></main>` + footer(root, lang);
}
...
function europeFaqPage(lang) {
  ...
  all.push(...(isDe ? [["Welche Informationen beschleunigen ein Angebot?", "Zielland und Ort, Nutzung, Personenanzahl, Wunschmodell oder Außenmaß, Budget, Termin, Grundstücksstatus sowie Fotos und Maße der letzten Zufahrt."], ["Bietet MODUNERA auch Modulbau, Stahlbau, Bungalows und Möbel an?", "Ja. Tiny Houses sind das Hauptgeschäft; die weiteren Leistungen werden projektbezogen angeboten."]] : [["Which details speed up a quotation?", "Country and place, intended use, occupants, preferred model or dimensions, budget, timing, plot status, and photos and measurements of final access."], ["Does MODUNERA also offer modular buildings, steel structures, bungalows and furniture?", "Yes. Tiny houses are the core business; additional services are offered on a project basis."]]));
...
function englishHome() {
  ...
  const description = "Eight premium tiny-house models, bespoke interiors and project-specific delivery to Germany, Netherlands, Denmark, Luxembourg and Switzerland.";
...
async function updateExistingText() {
  ...
  for (const file of files) {
    ...
    if (rel === "index.html") {
      ...
      const europeExpansionLinks = '<a href="laender/"><span>Europa</span><small>5 Zielmärkte</small></a><a href="leistungen/"><span>Weitere Bauten</span><small>Modul, Stahl, Bungalow, Möbel</small></a>';
...
async function writeSupportingFiles(markets, counts) {
  await put("site.webmanifest", JSON.stringify({ name: "MODUNERA", short_name: "MODUNERA", description: "Premium tiny houses and modular living for Europe", start_url: "/", display: "standalone", background_color: "#f8f3ed", theme_color: "#C29B72", icons: [{ src: "/assets/images/modunera-mark.png", sizes: "2048x2048", type: "image/png", purpose: "any maskable" }] }, null, 2) + "\n");
...
async function main() {
  ...
  const expected = ["laender/niederlande/index.html", "en/countries/netherlands/index.html", "leistungen/moebel-nach-mass/index.html", "blog/europa/tiny-house-transport-tuerkei-europa/index.html", "faq/europa/index.html", "assets/images/modunera-logo.png", "assets/images/modunera-mark.png"];
