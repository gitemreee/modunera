...
function guidePage(code, lang) {
  ...
  const root = rootFor(file);
  const name = country[isDe ? "de" : "en"];
  /* The series member says it is one. Germany already has a standalone permit
     pillar at /blog/tiny-house-genehmigung-deutschland/, and until 2026-08-29
     this page carried the same head title — two of our own URLs bidding on
     "tiny house genehmigung deutschland", both stranded around position 65.
     The other four countries have no such rival, so only Germany needs the
     distinction; it is applied by name rather than to all five, because
     retitling Denmark to avoid a collision that does not exist would be
     cargo-cult SEO. */
  const dePillar = code === "DE";
  const title = isDe
    ? (dePillar
        ? `Genehmigung in Deutschland im Ländervergleich | MODUNERA`
        : `Tiny House Genehmigung ${country.de}: Regeln & Checkliste | MODUNERA`)
    : (dePillar
        ? `Permits in Germany, compared across Europe | MODUNERA`
        : `Tiny house permits in ${country.en}: rules & checklist | MODUNERA`);
